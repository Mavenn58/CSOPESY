...
void displayProcess(const std::vector<Process>& processes)
{
    ...
    for (const auto& process : processes) {
        ...
        std::cout << "|" << std::setw(4) << process.gpuId << " " << std::setw(8) << process.graphicsId << " " << std::setw(8) << process.computeId << " " << std::setw(6) << process.pid << " " << std::setw(5) << process.type << "    " << std::setw(40) << std::left << name << " " << std::setw(9) << std::right << gpuMemoryUsage << "|\n";
    }
